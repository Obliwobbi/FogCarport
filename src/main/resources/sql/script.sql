...
INSERT INTO employees (name, email, phone, is_admin) VALUES
('Admin User', 'admin@fogcarport.dk', '+45 12345678', TRUE),
...
INSERT INTO materials (name, description, unit, unit_type, material_length, material_width, material_height, price) VALUES
('Brædt 25x200', '25x200 mm. trykimp. Brædt', 1, 'stk', 540.00, 20.00, 2.50, 300.00),
...
COMMENT ON TABLE employees IS 'Stores employee/admin information';
COMMENT ON TABLE customers IS 'Stores customer information with full address details';
COMMENT ON TABLE materials IS 'Stores available building materials with dimensions and prices';
COMMENT ON TABLE carports IS 'Stores carport specifications';
COMMENT ON TABLE drawings IS 'Stores technical drawings for carports';
COMMENT ON TABLE bills_of_materials IS 'Stores bill of materials with total prices';
COMMENT ON TABLE materials_lines IS 'Individual line items in a bill of materials';
COMMENT ON TABLE orders IS 'Stores customer orders linking all entities together';

COMMENT ON COLUMN materials.unit IS 'Quantity per unit (e.g., 1 for single item, 200 for pack of 200)';
COMMENT ON COLUMN materials.material_length IS 'Length in cm (NULL for items without length)';
COMMENT ON COLUMN materials.material_width IS 'Width in cm (NULL for items without width)';
COMMENT ON COLUMN materials.material_height IS 'Height/thickness in cm (NULL for items without height)';
COMMENT ON COLUMN customers.house_number IS 'House number (can include letters like 12A)';
COMMENT ON COLUMN materials_lines.line_price IS 'Total price for this line (quantity * unit price)';
